...
namespace Gedcom551.Constructed
{
    ...
    public class GedcomStructureSchema
    {
        ...
        public bool HasComplexPayloadType
        {
            ...
            {
#if true
                if (!this.AliasSpecification.Any())
                {
                    return false;
                }
                if (this.AliasSpecification.Count == 1 && this.AliasSpecification.First() == "<TEXT>")
                {
                    return false;
                }
                return true;
#else
                if (this.TypeSpecification.Any())
                {
                    string line = this.TypeSpecification.First();
                    if (line.StartsWith('<') &&
                        line != "<TEXT>")
                    {
                        return true;
        ...
        public string? GetAliasedPayloadType()
        {
            if (this.TypeSpecification.Any())
            {
                string line = this.TypeSpecification.First();
                ...
                {
                    string middle = line.Substring(1, line.Length - 2);
                    return middle;
                }
            }
            return null;
        ...
        GedcomStructureSchema(string sourceProgram, string tag)
        {
            ...
            this.AliasSpecification = new List<string>();
        ...
        GedcomStructureSchema(Dictionary<object, object> dictionary)
        {
            ...
            string uri = (dictionary["uri"] as string) ?? string.Empty;
            Debug.Assert(uri.StartsWith(UriPrefix));
            this._pinnedFinalRelativeUri = uri.Substring(UriPrefix.Length);
            
            this.StandardTag = (dictionary["standard tag"] as string) ?? string.Empty;
            this.Label = (dictionary["label"] as string) ?? string.Empty;
            this.OriginalPayload = (dictionary["payload"] as string) ?? string.Empty;
            ...
            this.TagSpecification = new List<string>();
            AddStrings(this.TagSpecification, dictionary["specification"] as Object[]);
            this.Substructures = new Dictionary<GedcomStructureSchema, GedcomStructureCountInfo>();
            AddDictionary(this.LoadedSubstructures, dictionary["substructures"] as Dictionary<object, object>);
            this.Superstructures = new List<GedcomStructureSchema>();
            AddDictionary(this.LoadedSuperstructures, dictionary["superstructures"] as Dictionary<object, object>);
        }
#endif
        ...
        /// <summary>
        /// Some types are aliases for other types.  For example,
        /// "CHANGE_DATE:= <DATE_EXACT>
        ///    Some English description."
        /// In such a case, "<DATE_EXACT>" is the alias specification.
        /// </summary>
        public List<string> AliasSpecification { get; private set; }

        /// <summary>
        /// English definition of type, not including the AliasSpecification,
        /// if any.
        /// </summary>
        ...
        /// <summary>
        /// Literal payload in the spec.  It may be another type.
        /// </summary>
        ...
        public string EffectivePayload
        {
            get
            {
                ...
                {
                    string trimmedPayload = payload.Substring(1, payload.Length - 2);
                    List<GedcomStructureSchema> others = GedcomStructureSchema.GetAllSchemasForPayload(trimmedPayload);
        ...
        public static void LoadAll(string gedcomRegistriesPath = null)
        {
            ...
            foreach (string filename in files)
            {
                ...
                s_StructureSchemasByUri[schema.AbsoluteUri] = schema;
                if (schema.IsRecord)
                {
                ...
                {
                    foreach (var superstructureSchema in schema.Superstructures)
                    {
                        string superstructureUri = superstructureSchema.AbsoluteUri;
        ...
        /// <summary>
        /// Given a list of strings, remove any leading and trailing blank lines.
        /// </summary>
            ...
            foreach (GedcomStructureSchema schema in s_StructureSchemas)
            {
                ...
                try
                {
                    ...
                    using (StreamWriter writer = new StreamWriter(filePath))
                    {
                        ...
                        if (string.IsNullOrEmpty(schema.EffectivePayload))
                        {
                            writer.WriteLine("null");
                        }
                        else if (schema.EffectivePayload == "[Y|<NULL>]")
                        {
                            writer.WriteLine("Y|<NULL>");
                        }
                        else if (schema.EffectivePayload.StartsWith("@<XREF:"))
                        {
                            string recordType = schema.EffectivePayload.Substring(7).Trim('@', '>');
                            writer.WriteLine("\"@<https://gedcom.io/terms/v5.5.1/record-" + recordType + ">@\"");
                        }
                        else if (schema.EffectivePayload.StartsWith("[@<XREF:") && schema.EffectivePayload.EndsWith(">@|<NULL>]"))
                        {
                            string recordType = schema.EffectivePayload.Substring(8, schema.EffectivePayload.Length - 18);
                            writer.WriteLine("\"@<https://gedcom.io/terms/v5.5.1/record-" + recordType + ">@|<NULL>\"");
                        }
                        else if (schema.EffectivePayload.StartsWith("[<") && schema.EffectivePayload.EndsWith(">|<NULL>]"))
                        {
                            string payloadType = schema.EffectivePayload.Substring(2, schema.EffectivePayload.Length - 11);
                            writer.WriteLine("https://gedcom.io/terms/v5.5.1/type-" + payloadType);
                        }
                        else if (schema.EffectivePayload.Contains('@') || schema.EffectivePayload.Contains('|'))
                        {
                            throw new Exception("Bad payload");
                        }
                        else if (!schema.EffectivePayload.StartsWith("http"))
                        {
                            writer.WriteLine("https://gedcom.io/terms/v5.5.1/type-" + schema.EffectivePayload);
                        }
                        else
                        {
                            writer.WriteLine(schema.EffectivePayload);
                        }
                        writer.WriteLine();
        ...
        private string ConstructFinalRelativeUri()
        {
            ...
            schemas = s_StructureSchemas.Where(s => s.StandardTag == tag && s.EffectivePayload == EffectivePayload && !s.IsRecord).ToList();
            if (schemas.Count == 1)
            {
                return GenerateTagPayloadRelativeUri(string.Empty, tag, EffectivePayload);
            }
            ...
            return GenerateTagPayloadRelativeUri(supers, tag, EffectivePayload);
        }
